    ...
    : juce::AudioProcessor(
      ...
      apvts(*this, nullptr, "Params",
            juce::AudioProcessorValueTreeState::ParameterLayout{
                ...
                std::make_unique<juce::AudioParameterFloat>(
                    "preDelay", "Pre-Delay", 0.0f, 100.0f, 5.0f),
...
void GranularReverbProcessor::prepareToPlay(double sampleRate, int blockSize)
{
    ...
    for (int ch = 0; ch < 2; ++ch)
    {
        ...
        preBuf[ch].clear();
        preBuf[ch].setSize(1, (int)(sampleRate * 0.2));
                                           ...
                                           juce::MidiBuffer&)
{
    ...
    auto* preDly   = apvts.getRawParameterValue("preDelay");
    auto* asym     = apvts.getRawParameterValue("asym");
    ...
    const int preDelaySamples = (int)(preDly->load() * sr / 1000.0f);
    const bool asymOn = asym->load() > 0.5f;
    ...
    if (stopFlag.load(std::memory_order_relaxed))
    {
        ...
        for (int ch = 0; ch < 2; ++ch)
        {
            ...
            apBuf[ch].clear();
            preBuf[ch].clear();
            for (int c = 0; c < NUM_COMBS; ++c) combState[ch][c] = 0.0f;
            apState[ch] = 0.0f;
        }
        toneStateL = 0.0f;
        toneStateR = 0.0f;
    ...
    // Asymétrie L/R
    float fbL = fb;
    float fbR = asymOn ? fb * (1.0f - width->load() * 0.08f) : fb;
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        // PRE-DELAY
        {
            ...
            int read = (prePos[0] - preDelaySamples + preLen * 4) % preLen;
            buf[prePos[0]] = swL;
            swL = buf[read];
        ...
        }
        {
            ...
            int read = (prePos[1] - preDelaySamples + preLen * 4) % preLen;
            buf[prePos[1]] = swR;
            swR = buf[read];
...
void GranularReverbProcessor::stopEffect()
{
